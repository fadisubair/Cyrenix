  ...
  return (
    <div className="space-y-6 flex flex-col h-full">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-wide">Identities</h1>
          <p className="text-gray-400">User Identity Risk Profiles and Behavioral Anomalies</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-panel border border-border p-6 rounded-lg flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-400">Total Monitored Identities</p>
            <p className="text-3xl font-bold text-white mt-2">{identities.length}</p>
          </div>
        ...
        <div className="bg-panel border border-border p-6 rounded-lg flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-400">Total Active Signals</p>
            <p className="text-3xl font-bold text-white mt-2">
      ...
      <div className="bg-panel border border-border rounded-lg overflow-hidden flex flex-col flex-1">
        {isLoading ? (
          <div className="p-12 text-center text-gray-400">Analyzing identity risks...</div>
        ) : (
                ...
                {identities.map(profile => (
                  <React.Fragment key={`profile-${profile.id}`}>
                    <tr 
                      ...
                      className={`hover:bg-white/[0.02] cursor-pointer transition-colors ${expandedRows[profile.id] ? 'bg-white/[0.02]' : ''}`}
                    >
                      <td className="px-4 py-4 text-gray-500">
                        {expandedRows[profile.id] ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                      </td>
                      <td className="px-4 py-4 text-sm font-medium text-white flex items-center">
                      ...
                      <tr className="bg-background/30">
                        <td colSpan={5} className="px-8 py-6 border-l-4 border-l-primary/50">
                          ...
                          {(!profile.signals || profile.signals.length === 0) ? (
                            <div className="text-sm text-gray-500 italic bg-background/50 p-4 rounded-md border border-border/50">
                              No active risk signals detected for this identity. Baseline behavior is normal.
                            </div>
                          ) : (
                            <div className="grid gap-3 grid-cols-1 lg:grid-cols-2">
                              {profile.signals.map(signal => (
                                <div key={signal.id} className="bg-background/80 border border-border/50 rounded-md p-4">
                                  <div className="flex justify-between items-start mb-2">
                                    <div className="font-medium text-sm text-white">{signal.signal_type}</div>
                                  ...
                                  {signal.evidence && (
                                    <div className="bg-[#0D1117] border border-border/50 rounded p-2 overflow-x-auto text-xs font-mono text-gray-300">
                                      {Object.entries(signal.evidence).map(([k, v]) => (
                                        <div key={k} className="flex">
                                          <span className="text-primary/70 min-w-[100px]">{k}:</span>
                          ...
                          <div className="mt-6 pt-4 border-t border-border/50">
                            ...
                            <div className="bg-[#0D1117] border border-border/50 rounded p-4 text-xs text-gray-400 font-mono">
