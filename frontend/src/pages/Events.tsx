  ...
  return (
    <div className="space-y-6 flex flex-col h-full">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-zinc-100 tracking-wide">Global Events</h1>
          <p className="text-zinc-400">Search and explore raw security telemetry</p>
        </div>
        <button 
          onClick={() => fetchEvents()}
          className="flex items-center px-4 py-2 bg-primary text-indigo-400-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-colors"
        >
      ...
      {/* Filter Panel */}
      <div className="bg-panel border border-border p-4 rounded-md flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <input 
            ...
            className="w-full bg-zinc-950 border border-border rounded-md px-3 py-1.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none"
          />
        </div>
        <div className="flex-1 min-w-[200px]">
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="block text-xs text-zinc-400 mb-1">Source IP</label>
          <input 
            type="text" 
            name="source_ip"
      ...
      <div className="bg-panel border border-border rounded-md overflow-hidden flex flex-col flex-1">
        {isLoading ? (
          <div className="p-8 text-center text-zinc-400">Querying events...</div>
        ) : (
                ...
                {events.map(event => (
                  <React.Fragment key={`event-${event.id}`}>
                    <tr 
                      ...
                      className={`hover:bg-zinc-800/30 cursor-pointer transition-colors ${expandedRows[event.id] ? 'bg-white/[0.02]' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3 text-sm">
                        {event.source_ip || event.destination_ip ? (
                          <div className="flex flex-col text-xs text-zinc-400 space-y-1">
                            {event.source_ip && <div><span className="text-zinc-500">SRC:</span> {event.source_ip}</div>}
                            {event.destination_ip && <div><span className="text-zinc-500">DST:</span> {event.destination_ip}</div>}
                          </div>
                        ) : <span className="text-zinc-600">-</span>}
                      </td>
                      <td className="px-4 py-3 text-sm">
                    ...
                    {expandedRows[event.id] && (
                      <tr className="bg-zinc-950/30">
                        <td colSpan={7} className="px-4 py-4">
                          ...
                          <div className="bg-zinc-950 border border-border/50 rounded-md p-4 overflow-x-auto">
